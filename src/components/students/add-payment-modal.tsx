...
import { cn, generateMonthYearOptions } from '@/lib/utils';
...
interface AddPaymentModalProps {
  ...
  onAddPayment: (studentId: string, payment: Omit<Payment, 'id'>) => void;
  monthlyFee: number;
  referenceDate: Date; 
}

export function AddPaymentModal({ isOpen, onClose, student, onAddPayment, monthlyFee, referenceDate }: AddPaymentModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const currentFee = student?.monthlyFee !== undefined ? student.monthlyFee : monthlyFee;
      form.reset({
        dateOfPayment: new Date(), 
        paymentMethod: undefined, 
        monthPaidFor: undefined, 
  ...
  }, [isOpen, student, monthlyFee, form, referenceDate]); 
  ...
  const onSubmit = async (data: PaymentFormValues) => {
    ...
    try {
      await onAddPayment(student.id, data);
    } finally {
      // setIsSubmitting(false); // Parent controls closing and submit state end potentially.
      // For safety, let's ensure isSubmitting is reset if onAddPayment doesn't cause unmount.
      // If onClose unmounts, this is not strictly needed.
      // If the modal stays open for some reason (e.g. error in onAddPayment not caught by parent),
      // we want the button to be re-enabled.
      // However, the parent FeeTrackerPage.tsx already handles closing logic which should reset isSubmitting implicitly
      // via useEffect when isOpen becomes false or when student/payment data changes.
      // The most direct control point for isSubmitting is before and after the async onAddPayment call.
      // The parent component (FeeTrackerPage) handles closing the modal which implicitly resets UI.
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open && !isSubmitting) onClose(); }}>
      <DialogContent className="w-[95vw] max-w-md sm:max-w-[525px] max-h-[85vh] flex flex-col overflow-hidden p-0 rounded-lg sm:rounded-lg bg-card backdrop-blur-md border-border">
        <DialogHeader className="pt-6 px-6 pb-4 shrink-0 border-b border-border/50">
          <DialogTitle className="font-headline text-2xl text-foreground">Add Payment for {student.fullName}</DialogTitle>
          <DialogDescription className="text-muted-foreground">Record a new payment for this student. Monthly fee: ₹{student.monthlyFee !== undefined ? student.monthlyFee : monthlyFee}</DialogDescription>
        </DialogHeader>
